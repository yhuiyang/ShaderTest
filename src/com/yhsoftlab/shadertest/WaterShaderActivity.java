...
public class WaterShaderActivity extends SimpleBaseGameActivity {
	...
	private ITextureRegion mWaterRegion;
	private ITextureRegion mDistanceRegion;
	...
	protected void onCreateResources() throws IOException {
		...
		mWaterRegion = BitmapTextureAtlasTextureRegionFactory.createFromAsset(
				texture, this, "water/water.png");
		mDistanceRegion = BitmapTextureAtlasTextureRegionFactory
				.createFromAsset(texture, this, "water/noise.png");
		try {
	...
	protected Scene onCreateScene() {
		...
		scene.getBackground().setColor(Color.BLUE);

		scene.attachChild(new WaterSurfaceEntity(mCAMERA.getCenterX(), mCAMERA
				.getCenterY(), new Sprite(0, 0, mWaterRegion, mEngine
				.getVertexBufferObjectManager()), new Sprite(0, 0,
				mDistanceRegion, mEngine.getVertexBufferObjectManager()),
				mEngine));
	...
	// ===========================================================
	public static class WaterSurfaceEntity extends Entity {
		...
		float Frequency = 30f; // bigger = more waves per pixels
		float Amplitude = 0.005f; // bigger = higher waves
		float speedMultiplicator = 5f;

		public WaterSurfaceEntity(float pX, float pY, Sprite pBackground,
				Sprite pDis, Engine pEngine) {
			super(pX, pY);

			engine = pEngine;
			waterTexture = new RenderTexture(engine.getTextureManager(),
					(int) pBackground.getWidth(),
					(int) pBackground.getHeight(), PixelFormat.RGBA_8888);
			displacementTexture = new RenderTexture(engine.getTextureManager(),
					(int) pBackground.getWidth(),
					(int) pBackground.getHeight(), PixelFormat.RGBA_8888);
		...
		protected void draw(final GLState pGLState, final Camera pCamera) {
			...
			if (first) {
				...
				final ITextureRegion renderTextureATextureRegion = TextureRegionFactory
						.extractFromTexture(waterTexture);

				RenderTextureBackgroundSprite = new Sprite(0, 0,
					...
					protected void preDraw(final GLState pGLState,
						...
						super.preDraw(pGLState, pCamera);
						GLES20.glUniform1f(WaterShaderProgram.sUniformTime,
								secondsElapsed);
						GLES20.glUniform1f(
								WaterShaderProgram.sUniformFrequency, Frequency);
						GLES20.glUniform1f(
								WaterShaderProgram.sUniformAmplitude, Amplitude);
					}
				};
			}
		...
		public static class WaterShaderProgram extends ShaderProgram {
			...
			public static final String FRAGMENTSHADER = "precision lowp float;\n"
					...
					+ "float t = "
					+ ShaderProgramConstants.VARYING_TEXTURECOORDINATES
					+ ".y + maskAlpha.y *0.1-0.15 + (sin ("
					+ ShaderProgramConstants.VARYING_TEXTURECOORDINATES
					+ ".x *frequency+timedelta)*amplitude);\n"
			...
			protected void link(final GLState pGLState)
				...
				WaterShaderProgram.sUniformModelViewPositionMatrixLocation = this
						.getUniformLocation(ShaderProgramConstants.UNIFORM_MODELVIEWPROJECTIONMATRIX);
				WaterShaderProgram.sUniformTexture0Location = this
						.getUniformLocation(ShaderProgramConstants.UNIFORM_TEXTURE_1);
				WaterShaderProgram.sUniformTexture1Location = this
						.getUniformLocation(ShaderProgramConstants.UNIFORM_TEXTURE_0);
				WaterShaderProgram.sUniformTime = this
						.getUniformLocation(WaterShaderProgram.TIME);
				WaterShaderProgram.sUniformFrequency = this
						.getUniformLocation(WaterShaderProgram.FREQUENCY);
				WaterShaderProgram.sUniformAmplitude = this
						.getUniformLocation(WaterShaderProgram.AMPLITUDE);
			}

			@Override
			public void bind(
					final GLState pGLState,
				...
				GLES20.glUniformMatrix4fv(
						WaterShaderProgram.sUniformModelViewPositionMatrixLocation,
						1, false, pGLState.getModelViewProjectionGLMatrix(), 0);
				GLES20.glUniform1i(WaterShaderProgram.sUniformTexture0Location,
						1);
				GLES20.glUniform1i(WaterShaderProgram.sUniformTexture1Location,
						0);
			}
